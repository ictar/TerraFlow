...
function buildCLIConfig() {
    ...
    if (loggerNode) {
        if (loggerNode.type === 'CustomLogger') {
            ...
            if (type === 'CSVLogger') classPath = "lightning.pytorch.loggers.CSVLogger";
    ...
    if (taskNode.type === 'PixelwiseRegressionTask') taskClass = 'terratorch.tasks.PixelwiseRegressionTask';
    ...
    if (neckNode) {
        if (neckNode.type === 'CustomNeck') {
            ...
            if (neckNode.data.reshape === 'True') necksConfig.push({ name: 'ReshapeTokensToImage' });
        }
    } else {
        // Default
    ...
    if (schedulerNode) {
        if (schedulerNode.type === 'CustomLRScheduler') {
            ...
            if (schedulerNode.data.type === 'ReduceLROnPlateau') {
                schedulerConfig = {
                    ...
                    init_args: { monitor: schedulerNode.data.monitor, patience: parseInt(schedulerNode.data.patience), mode: 'min' }
                };
            } else {
                schedulerConfig = {
        ...
        schedulerConfig = { class_path: "CosineAnnealingLR", init_args: { T_max: parseInt(trainerNode.data.max_epochs) } };
    }
    ...
    if (backboneNode) {
        ...
        if (backboneNode.type === 'CustomBackbone') {
            ...
            // Standard
            finalModelArgs.backbone_pretrained = backboneNode.data.pretrained === 'True';
    ...
    if (decoderNode) {
        ...
        if (decoderNode.type === 'CustomDecoder') {
            ...
            finalModelArgs.decoder_channels = parseInt(decoderNode.data.decoder_channels);
            finalModelArgs.decoder_scale_modules = decoderNode.data.scale_modules === 'True';
        }
    }
    ...
    if (headNode) {
        if (headNode.type === 'CustomHead') {
             ...
             finalModelArgs.learned_upscale_layers = parseInt(headNode.data.learned_upscale_layers);
             if (headNode.data.final_act !== 'None') finalModelArgs.final_act = `torch.nn.${headNode.data.final_act}`;
        }
    }
    ...
    const modelBlock = {
        ...
        init_args: {
            ...
            ignore_index: taskNode.data.ignore_index ? parseInt(taskNode.data.ignore_index) : undefined,
    ...
    const cliConfig = {
        ...
        data: {
            class_path: dataNode.type === 'CustomDataModule' ? dataNode.data.class_path : dataNode.data.class_path, // Standard has it too
